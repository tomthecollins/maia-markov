...
// distribution for tonal excerpts in 4-4 time for AI Eurovision 2020 project.

// Individual user paths.
const mainPaths = {
  "tom": {
    "midi": "/Users/tomthecollins/Shizz/Data/Music/imogen_heap/",
...
// Let's take the first 23 songs in 4-4 time. They transcribed the best.
let trRmk = require(mainPath["midi"] + "transcription_remarks")
console.log("trRmk.length:", trRmk.length)
trRmk = trRmk.filter(function(tr){
  return tr.timeSig == "4-4"
}).slice(0, 23)
// console.log("trRmk:", trRmk)
const trRmkNames = trRmk.map(function(tr){
  return tr.name
})
...
midiDirs.forEach(function(midiDir, jDir){
  ...
  pFiles = pFiles.filter(function(pFile){
    ...
    trRmkNames.indexOf(pFile.split(".")[0]) >= 0
  })
  ...
  fs.writeFileSync(
    mainPath["outputDir"] + "heap_" + midiDir + ".js",
    // "var perc_" + midiDir + " = " +
  ...
  fs.writeFileSync(
    mainPath["outputDir"] + "heap_" + midiDir + "_initial.js",
    // "var perc_" + midiDir + " = " +
